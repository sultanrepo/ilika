						...
						<div class="contactapp-detail-wrap">
							<header class="contact-header">
								<div class="d-flex align-items-center">
									<div class="dropdown">
										<a class="contactapp-title dropdown-toggle link-dark" data-bs-toggle="dropdown" href="#" role="button" aria-haspopup="true" aria-expanded="false">
											<h1>Monthly Payment Members List</h1>
										</a>
								...
								<div class="contact-options-wrap">	
									<a class="btn btn-icon btn-flush-dark btn-rounded flush-soft-hover no-caret d-sm-inline-block d-none" href="reportMonthBetween.php" data-bs-toggle="tooltip" data-placement="top" title="" data-bs-original-title="Refresh"><span class="icon"><span class="feather-icon"><i data-feather="refresh-cw"></i></span></span></a>
									...
									<div class="dropdown-menu dropdown-menu-end">
										<a class="dropdown-item" href="#"><span class="feather-icon dropdown-icon"><i data-feather="star"></i></span><span>Stared Contacts</span></a>
									...
									<div class="contact-list-view">
										<table id="datable_1" class="table nowrap w-100 mb-5">
											...
											<tbody>
												...
												$query3 = "SELECT * FROM `deposit` WHERE date_of_payment BETWEEN '$from_date' AND '$to_date'";
                                                //$query3 = "SELECT users_login.user_id, users_login.user_email, users_login.user_mob_number, users_login.user_status, user_details.user_name, user_details.user_parm_address, user_details.user_star_donator, user_details.user_profession, user_details.profile_pic FROM users_login INNER JOIN user_details";
												$result3 = mysqli_query($conn, $query3);
												while ( $rows3 = mysqli_fetch_array($result3) ) {
													// echo "<pre>";
													// print_r($rows1);
													
													$user_id			 = $rows3['user_id'];
                                                    $amount              = $rows3['amount'];
                                                    $payment_Method      = $rows3['payment_method'];
                                                    $payment_status      = $rows3['status'];   
                                                    $date_time           = $rows3['created_at'];
                                                    $result4 = mysqli_query($conn, "SELECT users_login.user_email, users_login.user_mob_number, user_details.user_name, user_details.profile_pic, user_details.user_star_donator FROM `users_login` INNER JOIN user_details WHERE users_login.user_id='$user_id' AND user_details.user_id='$user_id'");
                                                    $rows4   = mysqli_fetch_array($result4);
													$user_email          = $rows4['user_email'];
													$user_mob_number     = $rows4['user_mob_number'];
													$user_name           = $rows4['user_name'];
													$user_star_donator   = $rows4['user_star_donator'];
													$profile_pic         = $rows4['profile_pic'];
													
													//Since Member Details
													$result2     = mysqli_query($conn, "SELECT * FROM `membership_start_details` WHERE user_id='$user_id'");
													$rows2       = mysqli_fetch_array($result2);
													$memberSince = $rows2['date_time'];
												?>
												<tr>
													<td>
														<div class="d-flex align-items-center">
														...
														<div class="media align-items-center">
															<div class="media-head me-2">
																<div class="avatar avatar-xs avatar-rounded">
																	<img src="Uploads/ProfilePhotos/<?php echo $profile_pic; ?>" alt="user" class="avatar-img">
																</div>
															</div>
															<div class="media-body">
																<span class="d-block text-high-em"><?php echo $user_name; ?></span> 
															</div>
														</div>
													</td>
													<td><?php echo $user_mob_number; ?></td>
													<td><?php echo $user_star_donator; ?>⭐</td>
